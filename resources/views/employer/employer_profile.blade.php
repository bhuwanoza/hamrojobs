                        ...
                        <div class="box box--shadow">
                            <div class="box-header">
                                ...
                                <div class="float-right">
                                    <a href="{{ route('application.index') }}">View All</a>
                                </div>
                                <div class="clearfix"></div>
                            </div>
                            <div class="box-body">
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on("click", "#edit_company_detail", function (e) {
            e.preventDefault();
            var tempurl = "{{ route('company.edit') }}";
        ...
        $(document).on('submit', '#updateCompany', function (e) {
            ...
            var params = $('#updateCompany').serializeArray();
        ...
        $(document).on('change', '#logoForm', function (e) {
            e.preventDefault();
            var form = new FormData($('#logoForm')[0]);

            if ($('#logo_image').val()) {
                form.append('logo_image', $('input[type=file]')[0].files[0]);
            }
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on('change', '#coverForm', function (e) {
            e.preventDefault();
            var form = new FormData($('#coverForm')[0]);

            if ($('#cover_image').val()) {
                form.append('cover_image', $('input[type=file]')[0].files[0]);
            }
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('company.update-cover') }}",
                contentType: false,
                processData: false,
                data: form,
                beforeSend: function (data) {
